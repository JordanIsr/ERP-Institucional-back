...
export class VersionMallaService {
  ...
  async findActivaPorCarrera(carreraId: string) {
    const version = await this.versionMallaRepository.findOne({
      where: {
        carrera: { id: carreraId },
        estado: EstadoVersionMalla.ACTIVA,
      },
    });
    if (!version) {
      throw new NotFoundException(
        'La carrera no tiene una malla ACTIVA. Configura y activa su malla antes de asociarla al periodo.',
      );
    }
    return version;
  }
  ...
  async activar(id: string) {
    ...
    const niveles = await this.versionMallaRepository.manager
      .getRepository(Nivel)
      .find({
        where: { versionMalla: { id } },
    ...
    if (niveles.length === 0) {
      throw new BadRequestException(
        'No se puede activar la malla porque todavía no tiene niveles.',
      );
    }

    const nivelVacio = niveles.find((nivel) => !nivel.detalles?.length);
    if (nivelVacio) {
      throw new BadRequestException(
        `No se puede activar la malla: ${nivelVacio.nombre} no tiene asignaturas.`,
      );
    }
