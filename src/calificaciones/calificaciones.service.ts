    ...
    return this.dataSource.transaction(
      async (manager) => {
        ...
        if (valorAnterior === null) {
          throw new BadRequestException(
            'Secretaría solo puede corregir una nota previamente registrada por el docente.',
          );
        }

        if (dto.tipoNota === TipoNota.RECUPERACION) {
          const promedioNormal =
            (Number(detalle.notaParcial1) +
              Number(detalle.notaParcial2)) /
            2;
          ...
          if (promedioNormal >= 7) {
            throw new BadRequestException(
              'La materia está aprobada y no corresponde una nota de recuperación.',
            );
          }
        }
