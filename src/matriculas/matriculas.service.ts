...
export class MatriculasService {
  ...
  /**
   * Oferta completa para la matrícula directa de estudiantes nuevos.
   * Incluye también las opciones no disponibles para que Secretaría pueda
   * ver por qué una jornada no se puede seleccionar, en vez de ocultarla.
   */
  async ofertaInicial() {
    const ofertas = await this.periodoCarreraRepository.find({
      where: {
    ...
    for (const oferta of ofertas) {
      ...
      const paralelos = await this.paraleloRepository.find({
        where: {
          periodoCarrera: { id: oferta.id },
          nivel: { numero: 1 },
        },
        order: { nombre: 'ASC' },
      });

      const paralelosConCupos = await Promise.all(
        paralelos.map(async (paralelo) => {
          const cuposOcupados = await this.matriculaRepository.count({
            where: {
          ...
          const materiasConfiguradas =
            await this.asignaturaParaleloRepository.count({
              where: { paralelo: { id: paralelo.id } },
            });

          const materiasRequeridas = await this.dataSource
            .getRepository(DetalleMalla)
            .count({
              where: { nivel: { id: paralelo.nivel.id } },
            });

          const cuposDisponibles = Math.max(
            paralelo.cupoMaximo - cuposOcupados,
            0,
          );

          let motivoNoDisponible: string | null = null;
          if (materiasRequeridas === 0) {
            motivoNoDisponible = 'El primer nivel de la malla no contiene materias.';
          } else if (materiasConfiguradas < materiasRequeridas) {
            motivoNoDisponible = `Falta asignar docente a ${materiasRequeridas - materiasConfiguradas} materia(s).`;
          } else if (cuposDisponibles === 0) {
            motivoNoDisponible = 'El paralelo está lleno.';
          }
      ...
      if (oferta.versionMalla.estado !== EstadoVersionMalla.ACTIVA) {
        motivoNoDisponible = `La malla está en estado ${oferta.versionMalla.estado}; para estudiantes nuevos debe estar ACTIVA.`;
      } else if (paralelosConCupos.length === 0) {
        motivoNoDisponible = 'No existe un paralelo de primer nivel.';
      } else if (!paralelosConCupos.some((paralelo) => paralelo.disponible)) {
        motivoNoDisponible = 'No existen paralelos configurados con cupos disponibles.';
      }

      resultado.push({
        periodoCarreraId: oferta.id,
        carrera: oferta.carrera,
        periodo: oferta.periodo,
        versionMalla: oferta.versionMalla,
        centroEstudio: oferta.centroEstudio,
        jornada: oferta.jornada,
        paralelos: paralelosConCupos,
        cupoMaximo: paralelosConCupos.reduce(
          (total, paralelo) => total + paralelo.cupoMaximo,
          0,
        ),
        cuposOcupados: paralelosConCupos.reduce(
          (total, paralelo) => total + paralelo.cuposOcupados,
          0,
        ),
        cuposDisponibles: paralelosConCupos.reduce(
          (total, paralelo) => total +
            (paralelo.disponible ? paralelo.cuposDisponibles : 0),
          0,
        ),
        disponible: motivoNoDisponible === null,
        motivoNoDisponible,
...
async crearNuevaConEstudiante(
  dto: CreateMatriculaNuevaDto,
) {
  return this.dataSource.transaction(
    async (manager) => {
      ...
      /*
       * Si la ficha ya existe, verificamos que
       * no tenga historial de matrículas.
       *
       * Esto permite recuperar estudiantes cuya
       * ficha quedó creada durante un intento fallido.
       */
      if (estudiante) {
        ...
        if (matriculaAnterior) {
          throw new ConflictException(
            'El estudiante ya tiene historial de matrícula. Los estudiantes antiguos deben continuar mediante una solicitud.',
          );
        }
      ...
      return this.crearEnTransaccion(
        manager,
        {
...
private async crearEnTransaccion(
  ...
  dto: CreateMatriculaDto,
      ...
      const detalleMallaRepo = manager.getRepository(DetalleMalla);
      ...
      const paralelo = await paraleloRepo
...
if (
  ...
  throw new BadRequestException(
    'El paralelo está incompleto. Verifica su oferta académica, nivel y malla curricular.',
  );
}
      ...
      const materiasRequeridas = await detalleMallaRepo.count({
        where: { nivel: { id: paralelo.nivel.id } },
      });

      if (materiasRequeridas === 0) {
        throw new BadRequestException(
          'El primer nivel de la malla no tiene asignaturas configuradas.',
        );
      }

      if (asignaturasDelParalelo.length < materiasRequeridas) {
        throw new BadRequestException(
          `El paralelo está incompleto: faltan ${materiasRequeridas - asignaturasDelParalelo.length} materia(s) con docente asignado.`,
        );
      }
